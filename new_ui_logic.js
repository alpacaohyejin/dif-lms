...
const GROUPS=['메인 프로젝트 - 1조','메인 프로젝트 - 2조','메인 프로젝트 - 3조','메인 프로젝트 - 4조','메인 프로젝트 - 5조'];
const STUDIES=['포토샵 스터디','일러스트 스터디'];
const LOG_CATS=['회의록','작업 기록','결정 사항','공지','기타'];
function studyBadges(studies){
...
// ─── UTILS ───────────────────────────────────────────────
function toast(msg,type=''){const t=document.getElementById('toast');t.textContent=msg;t.className='toast show'+(type?' '+type:'');setTimeout(()=>t.className='toast',2800);}
...
function dUntil(ds){const n=new Date();n.setHours(0,0,0,0);return Math.ceil((new Date(ds)-n)/86400000);}
function rCls(r){return r==='학회장'?'r-chief':r==='부학회장'?'r-vp':r==='임원진'?'r-exec':'r-member';}
function canMng(u){return['학회장','부학회장','임원진'].includes(u.rank);}
function isAdmin(u){return['학회장','부학회장'].includes(u.rank);}
function esc(s){return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}
function avC(id){return'av'+(id%4);}
function closeModal(){document.getElementById('modal-root').innerHTML='';}
function openModal(html){const ov=document.createElement('div');ov.className='m-ov';ov.innerHTML=html;ov.addEventListener('click',e=>{if(e.target===ov)closeModal();});document.getElementById('modal-root').appendChild(ov);}
function gOpts(sel){return GROUPS.map(g=>`<option ${sel===g?'selected':''}>${esc(g)}</option>`).join('');}
function rOpts(sel){return RANKS.map(r=>`<option ${sel===r?'selected':''}>${r}</option>`).join('');}
...
function doLogout(){DB.currentUser=null;document.getElementById('app').style.display='none';document.getElementById('auth-screen').style.display='flex';}
...
function goPage(page){
  ...
  if(page==='assignments')renderAssignments('ongoing');
...
function renderDashboard(){
  ...
  if(tA&&tA.records.length>0){
    ...
    document.getElementById('s-att').textContent=Math.round(att/active.length*100)+'%';
    document.getElementById('s-att-sub').textContent=att+'/'+active.length+'명 출석';
  }else{document.getElementById('s-att').textContent='—';document.getElementById('s-att-sub').textContent='출석 정보 없음';}
  const now=new Date();now.setHours(0,0,0,0);
  const ongoing=DB.assignments.filter(a=>new Date(a.deadline)>=now);
  document.getElementById('s-ongoing').textContent=ongoing.length;
  const myOver=DB.assignments.filter(a=>new Date(a.deadline)<now&&!a.submissions.find(s=>s.userId===u.id));
  document.getElementById('s-overdue').textContent=myOver.length;
  document.getElementById('s-overdue-sub').textContent=myOver.length>0?myOver.length+'개 미제출':'모두 제출 완료';
  // 임박 과제
  ...
  renderCalendar();
  // 팀 현황
  const teamHTML=GROUPS.map(g=>{const ms=active.filter(m=>m.group===g);
  ...
  if(canMng(u)){
    ...
    ap.innerHTML=`<div class="card"><div class="c-hd"><div class="c-title">⚙️ 출석 코드 관리 (운영진)</div></div>
      <div style="display:flex;gap:7px;margin-bottom:.85rem;">
        <input id="att-code-inp" class="fc" style="max-width:220px;" placeholder="오늘의 출석 코드" value="${esc(code)}">
        <button class="btn btn-p" onclick="setTodayCode()">코드 설정</button>
        <button class="btn btn-o" onclick="openAttStatusModal()">출석 현황</button>
      </div>
      ${code?`<div style="background:var(--yellow-light);border:2px dashed var(--yellow);border-radius:var(--r-md);padding:1rem;text-align:center;margin-bottom:.85rem;">
        <div style="font-size:30px;font-weight:700;letter-spacing:8px;">${code}</div>
        <div class="tm">오늘의 출석 코드 — ${fmt(today)}</div>
      </div>`:''}
    </div>`;
    // 사유 불참 대기 패널
    renderExcusePanel();
...
function submitAttCode(){
  const code=document.getElementById('att-code-member').value.trim().toUpperCase();
  if(!code)return toast('코드를 입력해주세요.','err');
  if(code!==DB.todayCode)return toast('출석 코드가 올바르지 않습니다.','err');
  const today=new Date().toISOString().split('T')[0];
  const tA=DB.attendance.find(a=>a.date===today);if(!tA)return;
  tA.records.push({userId:DB.currentUser.id,status:'출석',time:new Date().toTimeString().slice(0,5)});
...
function submitExcuse(date,sessionId){
...
function processExcuse(rid,decision){
...
function openAttStatusModal(){
  ...
  const cls=st==='출석'?'as-ok':st==='지각'?'as-lt':st==='사유'?'as-ex':st==='결석'?'as-ab':'as-nn';
  return`<tr><td>${esc(m.name)}</td><td>${esc(m.group)}</td><td><span class="${cls}">${st}</span></td></tr>`;}).join('');
  openModal(`<div class="modal" style="max-width:520px;"><div class="m-hd"><div class="m-title">오늘 출석 현황 — ${fmt(today)}</div><button class="m-x" onclick="closeModal()">✕</button></div>
  <div class="m-bd"><div class="tw"><table><thead><tr><th>이름</th><th>그룹</th><th>상태</th></tr></thead><tbody>${rows}</tbody></table></div></div>
  <div class="m-ft"><button class="btn btn-o" onclick="closeModal()">닫기</button></div></div>`);
}

// ─── ASSIGNMENTS ──────────────────────────────────────────
let curAsgTab='ongoing';
function switchAsgTab(tab,btn){curAsgTab=tab;document.querySelectorAll('.tab-btn').forEach(b=>b.classList.remove('active'));if(btn)btn.classList.add('active');renderAssignments(tab);}
...
function openSubmitModal(id){const a=DB.assignments.find(x=>x.id===id);
...
function openNewAsgModal(){
  ...
  <div class="m-bd">
    ...
    <div class="fg"><label class="fl">대상 그룹</label>
      <select id="na-group" class="fc"><option value="전체">전체</option>${GROUPS.map(g=>`<option>${esc(g)}</option>`).join('')}</select></div>
...
function delAsg(id){
  ...
  DB.assignments.splice(DB.assignments.findIndex(a=>a.id===id),1);
  toast('삭제되었습니다.','');renderAssignments(curAsgTab);
}
...
function renderMembers(){
  ...
  tbody.innerHTML=ms.map(m=>`<tr>
    ...
    <td><span class="rank ${rCls(m.rank)}">${m.rank}</span></td>
...
function filterM(k,v){DB.mFilter[k]=v;renderMembers();}
function openMemberModal(userId){
  ...
  <div class="m-bd">
    ...
    <div class="fg"><label class="fl">프로젝트 조</label>
      <select id="em-group" class="fc"><option value="">— 미지정 —</option>${GROUPS.map(g=>`<option ${m&&m.group===g?'selected':''}>${esc(g)}</option>`).join('')}</select></div>
  ...
  <div class="m-bd">
    <div style="text-align:center;margin-bottom:1.2rem;">
      ...
      <div style="display:flex;gap:5px;justify-content:center;margin-top:4px;">
        <span class="rank ${rCls(m.rank)}">${m.rank}</span>
...
function getAsgStats(uid,group){
...
function fF(k,v){DB.fFilter[k]=v;renderFiles();}
function openUploadModal(){
  openModal(`<div class="modal"><div class="m-hd"><div class="m-title">📁 파일 업로드</div><button class="m-x" onclick="closeModal()">✕</button></div>
  <div class="m-bd">
    <div class="al al-i"><span>ℹ️</span><div>서버 배포 환경에서 실제 업로드가 가능합니다. 현재는 파일 정보를 기록합니다.</div></div>
    <div class="fg"><label class="fl">파일 선택</label><input type="file" id="uf-inp" class="fc" accept=".pdf,.psd,.ai,.png,.jpg,.zip" onchange="previewFile(this)"></div>
    <div id="uf-preview" style="display:none;background:var(--gray-50);border-radius:var(--r-sm);padding:.6rem;margin-bottom:.85rem;">
      <div style="font-size:13px;font-weight:500;" id="uf-name"></div><div class="tm" id="uf-size"></div></div>
    <div class="fg"><label class="fl">그룹</label>
      <select id="uf-group" class="fc"><option value="전체">전체</option>${GROUPS.map(g=>`<option>${esc(g)}</option>`).join('')}</select></div>
  ...
  <div class="m-ft"><button class="btn btn-o" onclick="closeModal()">취소</button><button class="btn btn-p" onclick="doUpload()">업로드</button></div>
  </div>`);
}
function previewFile(inp){if(!inp.files||!inp.files[0])return;const f=inp.files[0];document.getElementById('uf-preview').style.display='';document.getElementById('uf-name').textContent=f.name;document.getElementById('uf-size').textContent=(f.size/1024/1024).toFixed(2)+' MB';}
function doUpload(){
  const inp=document.getElementById('uf-inp');const group=document.getElementById('uf-group').value;const asgId=document.getElementById('uf-asg').value;
  if(!inp.files||!inp.files[0])return toast('파일을 선택해주세요.','err');
  const f=inp.files[0];const ext=f.name.split('.').pop().toUpperCase();
  const tm={'PDF':'PDF','PSD':'PSD','AI':'AI','PNG':'PNG','JPG':'PNG','ZIP':'기타'};
  DB.files.push({id:DB.files.length+1,name:f.name,type:tm[ext]||'기타',group,uploaderId:DB.currentUser.id,assignId:asgId?parseInt(asgId):null,uploadedAt:new Date().toISOString().split('T')[0],sizeMB:parseFloat((f.size/1024/1024).toFixed(2))});
  closeModal();toast(f.name+' 업로드 완료! ✅','ok');renderFiles();
}
  ...
  <div class="m-bd">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-bottom:.85rem;">
      ...
      <div class="fg" style="margin-bottom:0;"><label class="fl">그룹</label>
        <select id="lg-group" class="fc"><option value='전체' ${e&&e.group==='전체'?'selected':''}>전체</option>${GROUPS.map(g=>`<option ${e&&e.group===g?'selected':''}>${esc(g)}</option>`).join('')}</select></div>
    </div>
...
function delLog(id){if(!confirm('로그를 삭제하시겠습니까?'))return;DB.teamLogs.splice(DB.teamLogs.findIndex(l=>l.id===id),1);toast('로그가 삭제되었습니다.','');renderLogs();}
...
// ─── NOTIFY ───────────────────────────────────────────────
function renderNotify(){renderNSPanel();renderAbPanel();renderNotiLog();updateNotifyBadge();}
...
function addNoti(type,title,message){
...
function updateNotifyBadge(){const b=document.getElementById('nav-notify-badge');if(b)b.textContent=DB.notifications.length>0?DB.notifications.length:'';}
function sendNSNotify(id){
  const a=DB.assignments.find(x=>x.id===id);
  const elig=DB.users.filter(u=>u.status==='active'&&(a.group==='전체'||u.group===a.group));
  const ns=elig.filter(m=>!a.submissions.find(s=>s.userId===m.id));
...
function clearNotify(){if(!confirm('발송 내역을 모두 삭제하시겠습니까?'))return;DB.notifications=[];updateNotifyBadge();renderNotiLog();}
function previewKakao(){
...
function renderAdmin(){
  ...
  document.getElementById('admin-ranks').innerHTML=active.map(m=>
    `<div style="display:flex;align-items:center;justify-content:space-between;padding:.45rem 0;border-bottom:1px solid var(--gray-100);">
      <span style="font-size:13px;">${esc(m.name)}</span><span class="rank ${rCls(m.rank)}">${m.rank}</span>
    </div>`).join('');
  const now=new Date();now.setHours(0,0,0,0);
...
function approveMember(id){const m=DB.users.find(u=>u.id===id);m.status='active';toast(m.name+' 승인 완료.','ok');renderAdmin();}
function rejectMember(id){if(!confirm('거절하시겠습니까?'))return;DB.users.splice(DB.users.findIndex(u=>u.id===id),1);toast('거절되었습니다.','');renderAdmin();}
