...
const DB={ groups:[], studies:['포토샵 스터디', '일러스트 스터디'], files:[], teamLogs:[], notifications:[], excuses:[], 
  currentUser:null,
  currentSem:'2026 2학기',
...
window.initApp = function(){ if(!DB.groups) DB.groups=[]; if(!DB.studies || DB.studies.length===0) DB.studies=['포토샵 스터디', '일러스트 스터디'];
...
window.renderDashboard = function(){
  ...
  const myOver=(DB?.assignments||[]).filter(a=>(new Date(a.deadline).setHours(23,59,59,999) < now)&&!(a.submissions||[]).find(s=>String(s.userId) === String(u.id)));
...
window.renderAttendance = function(){
  ...
  // 학회원 패널
  const myRec=tA?(tA?.records||[]).find(r=>String(r.userId) === String(u.id)):null;
  const myEx=(DB?.excuses||[]).find(r=>String(r.userId) === String(u.id)&&r.date===today);
...
window.submitAttCode = function(){
  ...
  const u = DB.currentUser;
  if((tA?.records||[]).find(r=>String(r.userId) === String(u.id))) return toast('이미 출석체크 되었습니다.','err');
...
window.openAttStatusModal = function(){
  ...
  const rows=active.map(m=>{const r=tA?(tA?.records||[]).find(x=>String(x.userId) === String(m.id)):null;const st=r?r.status:'미출석';
  const cls=st==='출석'?'as-ok':st==='지각'?'as-lt':st==='사유'?'as-ex':st==='결석'?'as-ab':'as-nn';
...
window.renderAssignments = function(tab){
  ...
  let asgns=(DB?.assignments||[]).filter(a=>{const dl = new Date(a.deadline); dl.setHours(23, 59, 59, 999);const sub=(a.submissions||[]).find(s=>String(s.userId) === String(u.id));
  ...
  el.innerHTML=asgns.map(a=>{const sub=(a.submissions||[]).find(s=>String(s.userId) === String(u.id));const d=dUntil(a.deadline);
  ...
  el.innerHTML=(DB?.assignments||[]).map(a=>{
    ...
    return`<div class="card" style="margin-bottom:1rem;">
      ...
      <div style="display:flex;gap:3px;flex-wrap:wrap;">
        ${elig.map(m=>{const s=(a.submissions||[]).find(x=>String(x.userId) === String(m.id));return`<span class="badge ${s?'b-gr':'b-re'}">${esc(m.name)}</span>`;}).join('')}
      </div>
    </div>`;}).join('');
}
window.openSubmitModal = function(id){const a=(DB?.assignments||[]).find(x=>String(x.id) === String(id));
...
window.doSubmit = function(id){
  ...
  (DB?.assignments||[]).find(x=>String(x.id) === String(id)).submissions.push({userId:DB.currentUser.id,submittedAt:new Date().toLocaleString('ko'),file});
...
window.viewSubs = function(id){
  ...
  const elig=(DB?.users||[]).filter(u=>u.status==='active'&&(a.group==='전체'||u.group===a.group));
  const rows=elig.map(m=>{const s=(a.submissions||[]).find(x=>String(x.userId) === String(m.id));
...
window.delAsg = function(id) {
  if(!confirm('정말 이 과제를 삭제하시겠습니까?')) return;
  DB.assignments = (DB.assignments||[]).filter(a => String(a.id) !== String(id));
  toast('과제가 삭제되었습니다.', 'ok');
  if(typeof renderAssignments === 'function') {
      const activeTabBtn = document.querySelector('.tabs .tab-btn.active');
      renderAssignments(activeTabBtn ? (activeTabBtn.textContent.includes('진행') ? 'ongoing' : activeTabBtn.textContent.includes('제출') ? 'submitted' : 'overdue') : 'ongoing');
  }
};
...
async function delFile(id){
  const f=(DB?.files||[]).find(x=>String(x.id) === String(id));
  if(!f)return;
  ...
  DB.files.splice((DB?.files||[]).findIndex(x=>String(x.id) === String(id)),1);
...
window.saveLog = function(id){
  ...
  if(id&&id!=='null'){const l=(DB?.teamLogs||[]).find(x=>String(x.id) === String(id));Object.assign(l,{title,category:cat,group,content});}
...
window.delLog = function(id) {
  if(!confirm('해당 팀 로그를 삭제하시겠습니까?')) return;
  DB.teamLogs = (DB.teamLogs||[]).filter(l => String(l.id) !== String(id));
...
window.exportAttCSV = function(){
  ...
  const rows=active.map(m=>{
    ...
    const sts=sessions.map(s=>{const r=s.records.find(x=>String(x.userId) === String(m.id));const st=r?r.status:(s.records.length>0?'결석':'—');
...
window.updateBadges = function(){
  ...
  const now=new Date();now.setHours(0,0,0,0);
  const myP=(DB?.assignments||[]).filter(a=>new Date(a.deadline)>=now&&!(a.submissions||[]).find(s=>String(s.userId) === String(u.id))&&(a.group==='전체'||a.group===u.group)).length;
...
window.deleteGroup = function(groupName) {
  ...
  toast(`'${groupName}' 그룹이 삭제되었습니다.`, 'ok');
  if(typeof renderAdminGroups === 'function') renderAdminGroups();
  if(document.getElementById('page-members')?.classList.contains('active')) {
    if (typeof renderMembers === 'function') renderMembers();
  }
...
window.saveMember = function(id) {
    ...
    if (u) {
        ...
        if(studentId!==undefined) u.studentId = studentId;
        if(dept!==undefined) u.dept = dept;
...
window.openMemberModal = function(id) {
    ...
    let html = `<div class="modal fc" onclick="event.stopPropagation()">
            ...
            ${isNew ? `<div class="fg"><label class="fl">이름</label><input type="text" id="em-name" class="fc" value=""></div>
            <div class="fg"><label class="fl">이메일</label><input type="email" id="em-email" class="fc" value=""></div>
            <div class="fg"><label class="fl">학번</label><input type="text" id="em-sid" class="fc" value=""></div>
            <div class="fg"><label class="fl">학과</label><input type="text" id="em-dept" class="fc" value=""></div>` : 
            ...
            <div class="fg"><label class="fl">직급</label>
                <select id="em-rank" class="fc">
                    ${['학회장', '부학회장', '임원진', '학회원'].map(r => `<option ${m.rank === r ? 'selected' : ''}>${esc(r)}</option>`).join('')}
                </select>
            </div>
